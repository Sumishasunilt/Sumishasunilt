# Hi 👋 I'm Sumisha Sunil T

## Aspiring Cloud Professional | AWS | Linux | Python | Networking

Aspiring Cloud professional with a strong interest in AWS Cloud, Linux Administration, Networking, and IT Infrastructure.Fresher with hands-on training and project experience in Cloud Computing and Linux Administration. Skilled in cloud infrastructure concepts and AWS cloud services including EC2, S3, VPC, IAM, RDS, CloudFront, Load Balancing, Auto Scaling, and CloudWatch, with foundational knowledge of Python, SQL, and Networking. Completed hands-on cloud projects available on GitHub and currently building skills in cloud infrastructure and DevOps tools.

My projects include AWS EC2 web server setup, S3 static website hosting with CloudFront, and a 2-Tier Web Architecture project involving EC2, Load Balancing, Auto Scaling, RDS, VPC, and CloudWatch. I am continuously improving my knowledge of Linux Administration, Networking, Python, AWS, and DevOps tools.

I am looking for entry-level opportunities in Cloud Computing, AWS, Linux Administration, Technical Support, System Administration, or IT Infrastructure where I can apply my skills, gain industry experience, and grow as an IT professional.

- 🌍 I'm based in Banglore 
- ✉️ You can contact me at **sumishasumil52@gmail.com**                                                                                                          

<p>
  <a href="https://git-scm.com/"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/git-colored.svg" width="30"/></a>
  <a href="https://www.linux.org/"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/linux-colored.svg" width="30"/></a>
  <a href="https://ubuntu.com/"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/ubuntu-colored.svg" width="30"/></a>
  <a href="https://aws.amazon.com/"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/aws-colored-dark.svg" width="30"/></a>
  <a href="https://www.python.org/"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/python-colored.svg" width="30"/></a>
  <a href="https://www.docker.com/"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/docker-colored.svg" width="30"/></a>
  <a href="https://kubernetes.io/"><img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/skills/kubernetes-colored.svg" width="30"/></a>
</p>

### Socials

<p align="left">
  <a href="https://github.com/Sumishasunilt">
    <img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/github.svg" width="25" height="25"/>
  </a>&nbsp;&nbsp;
  <a href="https://www.linkedin.com/in/sumisha-sunil-t/">
    <img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/linkedin.svg" width="25" height="25"/>
  </a>
</p>



## 🛠️ Skills

**Cloud & AWS**  
Amazon EC2 • Amazon S3 • VPC • IAM • RDS • CloudFront • ELB • Auto Scaling • CloudWatch

**Linux & Networking**  
Linux • Ubuntu • Amazon Linux • SSH • TCP/IP • DNS • HTTP/HTTPS • CCNA

**Programming & Database**  
Python • SQL 

**DevOps & Tools**  
Git • GitHub • AWS CLI • Docker • Jenkins • Terraform • CI/CD

## 🚀 Projects

### ☁️ AWS 2-Tier Web Architecture
Scalable cloud architecture using EC2, Load Balancing, Auto Scaling, RDS, VPC, Security Groups, and CloudWatch.

🔗 [View Project](https://github.com/Sumishasunilt/aws-2-tier-web-architecture)

### 🌐 AWS S3 Static Website Hosting with CloudFront
Static website hosting using Amazon S3 with CloudFront for content delivery and caching.

🔗 [View Project](https://github.com/Sumishasunilt/aws-s3-static-website)

## 📜 Certifications

- AWS Cloud Computing Certification — Besant Technologies
- Cloud Computing and DevOps — NASSCOM / IT-ITeS Sector Skills Council
- CCNA Training — Besant Technologies
